# ──────────────────────────────────────────────────────────
# 02_import_dess.R — Import DESS articles from browser export
#
# The DESS site (dess.gov.ua) uses WordPress + Elementor, so
# articles are fetched via a browser console script that
# produces dess_articles.json. This file imports that JSON
# into the standard corpus format.
#
# Usage:
#   source("R/01_config.R")
#   source("R/02_import_dess.R")
#   import_dess_articles("data/dess_articles.json")
# ──────────────────────────────────────────────────────────
...
#' Import DESS articles from browser-exported JSON
#'
#' @param json_path Path to dess_articles.json (from browser console script)
#' @param overwrite If FALSE, skip articles already in corpus
#' @return A tibble of imported articles
import_dess_articles <- function(json_path = file.path("data", "dess_articles.json"),
                                  overwrite = FALSE) {
  ...
  cat(sprintf("Total DESS articles in corpus: %d\n", saved + skipped))
