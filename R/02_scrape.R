# ──────────────────────────────────────────────────────────
# 02_scrape.R — Scrape presidential speeches & press releases
#
# Usage:
#   source("R/01_config.R")
#   source("R/02_scrape.R")
#   docs <- scrape_category("speeches", DATE_START, DATE_END)
#   # or:
    ...
    tryCatch({
      ...
      if (http_error(resp)) {
        message(sprintf("  [attempt %d/%d] HTTP %d for %s",
                        attempt, retries, status_code(resp), url))
...
parse_listing_page <- function(html_text) {
  ...
  if (is.null(articles) || length(articles) == 0) {
    ...
    for (link in links) {
      ...
      items[[length(items) + 1]] <- list(
        title = title, url = full_url, date_text = ""
      )
    }
  } else {
    for (art in articles) {
  ...
  if (!is.na(next_link)) {
    ...
    if (!is.na(href)) {
      next_url <- if (startsWith(href, "http")) href else paste0(BASE_URL, href)
    }
  }
  if (is.na(next_url)) {
